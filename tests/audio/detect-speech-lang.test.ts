...
describe('detectSpeechLang', () => {
  ...
  it('uses the UI-locale fallback for non-CJK text and leaves CJK / Vietnamese unchanged', () => {
    expect(detectSpeechLang('Photosynthesis is how plants grow', 'tr-TR')).toBe('tr-TR');
    expect(detectSpeechLang('Bonjour le monde', 'de-DE')).toBe('de-DE');
    expect(detectSpeechLang('', 'ja-JP')).toBe('ja-JP');
    expect(detectSpeechLang('光合作用是植物生长的基础', 'tr-TR')).toBe('zh-CN');
    expect(detectSpeechLang('Xin chào cả lớp!', 'tr-TR')).toBe('vi-VN');
  });
});
